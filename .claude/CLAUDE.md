...
**CSV Writers** (`pkg/csvplan/writer.go`): `WriteCSV(path, headers, rows, delimiter)` and `WriteYAML(path, rows)` write collection plan files with atomic writes (temp file + rename). `ReadHeaders(path)` in `collection.go` captures raw header list and delimiter for round-trip preservation.
...
- **Dynamic collection columns**: `discoverColumns()` scans all rows' `CustomFields` for non-empty values to determine which columns to show. Known fields (title, artist, name, start_time, duration) appear first in order; remaining fields sorted alphabetically. Flex columns capped at 45 chars max width.
