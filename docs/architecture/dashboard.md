...
- **Timeline / sequence edits**: `saveConfigAndReResolve` calls `config.Save(m.pp.ConfigFile, m.cfg)` to persist `powerhour.yaml`, then `reResolve` re-runs `project.ResolveTimeline` against the saved config so the preview reflects the new sequence.
...
- **`cache_doctor.go`** — the `overlayDoctor` overlay: inline metadata review/correction with fuzzy artist autocomplete and an async yt-dlp requery (`Ctrl+R`).
