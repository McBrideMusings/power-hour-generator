...
- **`timeline_view.go`** — two panels over one project: a read-only, fixed-height TIMELINE SEQUENCE panel (`timeline.sequence` from `powerhour.yaml` — genuinely editable only there, `e` opens it) and a mutable PLAYBACK ORDER panel showing what `finalize` would actually produce, resolved from the playback order via the same `playback.ResolveOrder`/`playback.Placements` call `finalize` itself uses (ADR 0003). The order panel's `s`/`l`/`S`/`Enter`/`Esc` gestures call `internal/playback` directly and hold no logic of their own (ADR 0003). `s` arms one of two modes, chosen by the collection's `selection` and never by its name (ADR 0002): a `repeat` collection's slot enters **cycle mode**, where `‹ ›` arrows wrap the label and `←`/`→` step its occupant through the pool in memory until `Enter` commits or `Esc` undoes; a `once` collection's slot is **marked** (`⇅`), the cursor then walks only that collection's slots, and `s` or `Enter` swaps the two. `l` locks, `S` shuffles. Also renders the finalized output path/status.
